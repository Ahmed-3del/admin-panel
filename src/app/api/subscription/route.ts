...
export const GET = withErrorHandling(async () => {
  ...
  const apiRes = await fetchWithThrow(
    `${process.env.NEXT_PUBLIC_API_URL}mailing-list`,
    {
      method: "GET",
      headers: {
  ...
  if (!apiRes.ok) {
    return new Response("Failed to fetch subscription", { status: apiRes.status });
  }
  const subscriptionData = await apiRes.json();
